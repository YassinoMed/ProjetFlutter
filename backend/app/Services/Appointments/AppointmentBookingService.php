...
class AppointmentBookingService
{
    public function createRequested(
        ...
        if ($startsAtUtc->lte(now('UTC'))) {
            throw ValidationException::withMessages([
                'starts_at_utc' => ['Start time must be in the future'],
            ]);
        }

        return DB::transaction(function () use ($patientUserId, $doctorUserId, $startsAtUtc, $endsAtUtc, $metadataEncrypted) {
            $doctor = User::query()->where('id', $doctorUserId)->lockForUpdate()->first();

            if ($doctor === null || $doctor->role !== UserRole::DOCTOR) {
                throw ValidationException::withMessages([
                    'doctor_user_id' => ['Doctor not found'],
                ]);
            }
